...
const TakeQuiz = ({ quizId, onBack, onComplete }) => {
    ...
    const exitFullscreen = () => {
        if (document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement) {
            ...
            if (exit) {
                exit.call(document).catch(err => console.error('Error exiting fullscreen:', err));
            }
        }
    };

    const handleExitFullscreenAndBack = () => {
        exitFullscreen();
        onBack();
    };
                    ...
                    <Card style={{ padding: 'var(--space-2xl)' }}>
                        <div className="flex flex-col items-center text-center">
                            ...
                            <div className="flex gap-md w-full">
                                <Button variant="secondary" onClick={handleExitFullscreenAndBack} style={{ flex: 1 }}>
                                    Not Now
                                </Button>
                                <Button onClick={handleStartQuiz} style={{ flex: 2 }}>
                                    Start Quiz
                                </Button>
                            </div>
                        </div>
    ...
    return (
        <div id="take-quiz-container" className="take-quiz-fullscreen-wrapper" style={{ width: '100%', minHeight: '100%', boxSizing: 'border-box' }}>
            {renderContent()}
        </div>
    );
};
