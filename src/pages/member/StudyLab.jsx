...
const StudyLab = () => {
    ...
    const handlePrint = (type, pageNum, originalText = '') => {
        ...
        const content = material?.content || 'No text content recorded for this material.';
        
        const pages = content.split(/\n\s*\n/); 
        let printContent = '';

        if (type === 'all') {
            printContent = pages.join('<div style="page-break-after: always; height: 1px;"></div>');
        ...
        const htmlStr = `
            ...
            <html>
                <head>
                    <title>Zenith Lab - ${title}</title>
                    <style>
                        body { font-family: system-ui, -apple-system, sans-serif; line-height: 1.6; color: #111827; padding: 30px; max-width: 800px; margin: 0 auto; }
                        h1 { color: #111827; border-bottom: 2px solid #e5e7eb; padding-bottom: 10px; font-size: 24px; }
                ...
                <body>
                    ...
                    <div class="footer">Generated by Zenith Study Lab · ${new Date().toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}</div>
                </body>
            </html>
        `;
        ...
        try {
            ...
            if (printWindow) {
                ...
                // Fallback using hidden printing iframe
            ...
            console.error('[StudyLab] Print failed:', err);
            window.print();
        }

        const messageContent = originalText || `Print ${type === 'all' ? 'all pages' : `page ${pageNum}`}`;
        setMessages(prev => [...prev, { role: 'user', content: messageContent }, { role: 'assistant', content: `🖨️ Preparation complete. Sent ${type === 'all' ? 'the full document' : `page ${pageNum}`} to printer.` }]);
    };
                                        ...
                                        <div style={mockDeviceMode ? {
                                            ...
                                            {mockDeviceMode && (
                                             ...
                                             <div style={{
                                                 ...
                                                 <span>ℹ️</span>
                                                 <span><strong>Notice:</strong> Browser canvas is designed to view study documents & verified external links. Some third-party websites may block embedding due to security origin rules.</span>
                                             </div>
